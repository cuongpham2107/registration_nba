...
<html lang="vi">
  <head>
    ...
    <style>
      ...
      @font-face {
        font-family: 'DejaVu Sans';
        src: url('{{ asset("fonts/DejaVuSans-Bold.ttf") }}') format('truetype');
        font-weight: bold;
        font-style: normal;
      }
    ...
    <div class="ticket-wrap">
      <div class="ticket">
        ...
        <div class="ticket-header">
            @if(file_exists($logo))
               <img src="data:image/png;base64,{{ base64_encode(file_get_contents($logo)) }}" alt="Company Logo" class="logo" />
            @else
               <div class="logo flex items-center justify-center text-xs" style="background:#d1d5db;">LOGO</div>
            @endif
          <div class="org-name">CÔNG TY CỔ PHẦN LOGISTICS ASG</div>
        </div>
        ...
        <div class="ticket-data">
            <div class="data-row">
                ...
                <div class="data-value">T{{now()->format('m')}}-{{ str_pad($record->id ?? 0, 2, '0', STR_PAD_LEFT) }}</div>
            </div>
            <div class="data-row">
                <div class="data-label">BKS xe</div>
                <div class="data-value" style="font-weight:700;">{{ $record->license_plate ?? 'N/A' }}</div>
            </div>
            <div class="data-row">
